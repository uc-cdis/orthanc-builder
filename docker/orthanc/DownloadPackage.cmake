  ...
  if (NOT EXISTS "${TMP_PATH}")
    ...
    if ("${MD5}" STREQUAL "no-check")
      ...
      if (NOT "${ActualMD5}" STREQUAL "${MD5}")
        message(FATAL_ERROR "The MD5 hash ${ActualMD5} (expected ${MD5}) of a previously download file is invalid: ${ActualMD5}")
      endif()
    endif()
  endif()
endmacro()
...
macro(DownloadPackage MD5 Url TargetDirectory)
  if (NOT IS_DIRECTORY "${TargetDirectory}")
    ...
    if (NOT IS_DIRECTORY "${TargetDirectory}")
      execute_process(COMMAND ls . OUTPUT_VARIABLE LS_OUTPUT OUTPUT_STRIP_TRAILING_WHITESPACE)
      message(FATAL_ERROR "The package was not uncompressed at the proper location (${TargetDirectory}). Check the CMake instructions. List: ${LS_OUTPUT}")
    endif()
    file(REMOVE ${TMP_PATH})
  endif()
endmacro()
