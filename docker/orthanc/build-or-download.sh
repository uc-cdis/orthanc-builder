...
if [[ $target == "orthanc" ]]; then
    ...
    if [[ $dl != 0 ]]; then
        ...
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DSTANDALONE_BUILD=ON -DUSE_GOOGLE_TEST_DEBIAN_PACKAGE=ON -DUSE_SYSTEM_CIVETWEB=OFF -DUSE_SYSTEM_DCMTK=OFF -DUSE_SYSTEM_OPENSSL=OFF -DUSE_SYSTEM_CURL=OFF $sourcesRootPath/OrthancServer
        make -j 4
    ...
    # if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF $sourcesRootPath
        make -j 4
        $buildRootPath/UnitTests
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DPYTHON_VERSION=3.11 $sourcesRootPath
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DSTATIC_BUILD=ON $sourcesRootPath
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF  $sourcesRootPath/PostgreSQL
        # cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DORTHANC_FRAMEWORK_SOURCE=path -DORTHANC_FRAMEWORK_ROOT=/orthanc/OrthancFramework/Sources -DORTHANC_SDK_VERSION=framework $sourcesRootPath/PostgreSQL
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF  $sourcesRootPath/MySQL
        # cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DORTHANC_FRAMEWORK_SOURCE=path -DORTHANC_FRAMEWORK_ROOT=/orthanc/OrthancFramework/Sources -DORTHANC_SDK_VERSION=framework $sourcesRootPath/MySQL
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF  $sourcesRootPath/Odbc
        # cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DORTHANC_FRAMEWORK_SOURCE=path -DORTHANC_FRAMEWORK_ROOT=/orthanc/OrthancFramework/Sources -DORTHANC_SDK_VERSION=framework $sourcesRootPath/Odbc
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DUSE_SYSTEM_LIBCSV=OFF $sourcesRootPath
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DUSE_SYSTEM_NIFTILIB=OFF $sourcesRootPath
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DUSE_SYSTEM_VTK=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_ORTHANC_SDK=OFF -DUSE_SYSTEM_NIFTILIB=OFF $sourcesRootPath
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_ORTHANC_SDK=OFF -DPLUGIN_VERSION=$extraArg1 $sourcesRootPath/orthanc-explorer-2/
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_ORTHANC_SDK=OFF $sourcesRootPath
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        cmake -DCMAKE_BUILD_TYPE:STRING=Release -DALLOW_DOWNLOADS=OFF -DUSE_SYSTEM_ORTHANC_SDK=OFF -DUSE_VCPKG_PACKAGES=OFF $sourcesRootPath/orthanc-object-storage/Aws/
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        cmake -DCMAKE_BUILD_TYPE:STRING=Release -DALLOW_DOWNLOADS=OFF -DUSE_SYSTEM_ORTHANC_SDK=OFF -DCMAKE_TOOLCHAIN_FILE=/vcpkg/scripts/buildsystems/vcpkg.cmake $sourcesRootPath/orthanc-object-storage/Google/
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        cmake -DCMAKE_BUILD_TYPE:STRING=Release -DALLOW_DOWNLOADS=OFF -DUSE_SYSTEM_ORTHANC_SDK=OFF -DCMAKE_TOOLCHAIN_FILE=/vcpkg/scripts/buildsystems/vcpkg.cmake $sourcesRootPath/orthanc-object-storage/Azure/
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF $sourcesRootPath
        make -j 4
        $buildRootPath/UnitTests
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DUSE_SYSTEM_OPENJPEG=OFF $sourcesRootPath/ViewerPlugin
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake -DALLOW_DOWNLOADS=OFF -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DORTHANC_STONE_BINARIES=/downloads/wasm-binaries/StoneWebViewer $sourcesRootPath/Applications/StoneWebViewer/Plugin/
        make -j 4
